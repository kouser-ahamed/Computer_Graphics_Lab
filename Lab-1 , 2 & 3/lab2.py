...
# Added global list to store generated DDA points
points = []
...
def dda_line(x1, y1, x2, y2):
    ...
    Generates intermediate points between (x1, y1) and (x2, y2)
    and plots them using OpenGL, also stores them in the list 'points'.
    """

    global points
    points.clear()  # clear previous data if any
    ...
    if steps == 0:
        ...
        glVertex2f(x1, y1)
        glEnd()
        points.append((round(x1), round(y1)))  # store single point
        return
...
def display():
    ...
    # Print generated points in console
